...
const Login = () => {
  ...
  const handleSubmit = async e => {
    ...
    setError('');
    try {
      // ری‌کوئست لاگین حالا فقط accessToken و user برمی‌گرداند
      const { accessToken, user } = await loginRequest(username, password);
      localStorage.setItem('token', accessToken);
      localStorage.setItem('user', JSON.stringify(user));
      // با جایگزینی در تاریخچه، اجازه نمی‌دهد کاربر با دکمهٔ Back برگردد
  ...
  return (
    <AuthLayout bgImage={wave}>
      ...
      <form onSubmit={handleSubmit} className="w-100">
        ...
        <p className="forgot-link mt-2" onClick={() => navigate('/forgot-password')}>
          فراموشی رمز عبور؟
        </p>
      </form>
    </AuthLayout>
